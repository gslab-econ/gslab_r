...
test.exampleModel.exampleModel <- function() {
  a <- exampleModel("y", c("x1", "x2"), suffix = "_c", startparam = c(2, 2, 2))
  ...
  checkException(exampleModel("y", "x", constant = 2), silent = TRUE)
  checkException(exampleModel("y", "x", constant = 0, startparam = c(2, 2)), silent = TRUE)
}
